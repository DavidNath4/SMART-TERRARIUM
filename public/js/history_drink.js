...
const deviceTemplate = (drink, date) => {
    return `
        <div class="log-container">
            <p class="value">${drink ? 'Available' : 'Unavailable'}</p>
            <p class="time">${date}</p>
        </div>
    `;
};
...
const terrariumDataLoader = (data) => {
    ...
    data.forEach((d) => {
        ...
        terrariumContainer.insertAdjacentHTML(
            "beforeend",
            deviceTemplate(d.drink, formatDate(d.createdAt))
        );

    });
};

generalDataLoader({ url: `/history/drink/${deviceID}`, func: terrariumDataLoader });
