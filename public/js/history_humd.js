...
const deviceTemplate = (humd, date) => {
    return `
        <div class="log-container">
            <p class="value">${humd}%</p>
            <p class="time">${date}</p>
        </div>
    `;
};
...
const terrariumDataLoader = (data) => {
    ...
    data.forEach((d) => {
        ...
        terrariumContainer.insertAdjacentHTML(
            "beforeend",
            deviceTemplate(d.humd, formatDate(d.createdAt))
        );

    });
};

generalDataLoader({ url: `/history/humd/${deviceID}`, func: terrariumDataLoader });
